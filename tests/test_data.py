...
def test_sample_batch_shape_and_range(tmp_path):
    ...
    arr.tofile(bin_path)
    mm = np.memmap(bin_path, dtype=np.uint16, mode="r", shape=(100,))
...
def test_sample_batch_y_is_x_shifted_by_one(tmp_path):
    ...
    arr.tofile(bin_path)
    mm = np.memmap(bin_path, dtype=np.uint16, mode="r", shape=(200,))
...
def test_sample_batch_reproducible_with_same_seed(tmp_path):
    ...
    arr.tofile(bin_path)
    mm = np.memmap(bin_path, dtype=np.uint16, mode="r", shape=(500,))
